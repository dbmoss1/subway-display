...
- 6x Adafruit 64x32 RGB LED Matrix panels, 2.5mm pitch ([product 5036](https://www.adafruit.com/product/5036)), wired as a single serpentine ("U-shaped") chain off the Bonnet's one output — **done**, actual physical order is bonnet → top-right → top-middle → top-left → bottom-left → bottom-middle → bottom-right, folded into a 192x64 display by the `rpi-rgb-led-matrix` "U-mapper" pixel mapper. This starts from the opposite corner U-mapper assumes (top-left), so `config.PIXEL_MAPPER` also applies a horizontal mirror (`"U-mapper;Mirror:H"`) to compensate. **Wiring order matters**: the ribbon between rows must land on the same side the first row's chain ends on, or the image scrambles.
- Adafruit RGB Matrix Bonnet ([product 3211](https://www.adafruit.com/product/3211)) — note this board has a single chain output only (no parallel-chain support), which is why the 6 panels must be one folded chain rather than 2 independent chains of 3 — **done**, mounted and attached
- Power: one 5V/10A supply ([product 658](https://www.adafruit.com/product/658)) through a 4-way DC splitter ([product 1352](https://www.adafruit.com/product/1352)) — **done**, 3 outputs each feed a pair of panels' bare wires via product-368 screw terminal blocks, 1 output feeds the Pi (via barrel-to-USB adapter), which powers the Bonnet through its GPIO pins
